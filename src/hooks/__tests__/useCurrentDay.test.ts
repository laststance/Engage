...
describe('useCurrentDay', () => {
  ...
  test('changes Today to the next local date at midnight while the app stays open', async () => {
    // Arrange
  ...
  test('exposes same-day foreground returns so daily assignments can retry', async () => {
    // Arrange
    const { result } = await renderHook(() => useCurrentDay())
    await act(async () => {
  ...
  test('uses the current local date when returning after multiple days away', async () => {
    // Arrange
    const { result } = await renderHook(() => useCurrentDay())
    await act(async () => {
      AppState.currentState = 'background'
  ...
  test('keeps the snapshot stable when native events leave the day and visibility unchanged', async () => {
    // Arrange
    const { result } = await renderHook(() => useCurrentDay())
  ...
  test('shares native resources and releases them after the last day consumer unmounts', async () => {
    // Arrange
